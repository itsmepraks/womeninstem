/**
 * Zustand store for managing book state across components
 */
    ...
    (set, get) => ({
      ...
      nextPage: () => {
        ...
        if (currentPage < totalPages - 1) {
          get().setCurrentPage(currentPage + 2); // Turn two pages at once
        }
      },
      ...
      previousPage: () => {
        ...
        if (currentPage > 0) {
          get().setCurrentPage(Math.max(0, currentPage - 2));
        }
      },
      ...
      addToHistory: (page) =>
        set((state) => ({
          history: [...state.history.slice(-10), page], // Keep last 10
        })),
